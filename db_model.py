...
def create_request(item,user,w_day,start_time,end_time):
    ...
    if (results <= 3):
      sql = "insert into request(item,user,w_day,start_time,end_time,approved) values (%d,%s,%s,"+start_time+","+end_time+",false); commit;"
      results = query_db(sql, agrs=(item,user,w_day))
    else:
